...
const DEFAULT_BASE_URL = "http://192.168.10.2:8674";
...
const OPENCLAW_INTERNAL_CONTEXT_BLOCK_RE =
  /<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>[\s\S]*?<<<END_OPENCLAW_INTERNAL_CONTEXT>>>\s*/g;
...
const AGENT_OVERRIDABLE_KEYS = [
  "userId", "conversationId", "conversationIdPrefix", "conversationIdSuffix",
...
export async function searchMemory(cfg, payload) {
  return callApi(cfg, "/product/search", sanitizeSearchPayload(payload));
}
...
function stripOpenClawInternalContext(text) {
  if (!text || typeof text !== "string") return text;
  if (!text.includes("<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>")) return text;
  return text.replace(OPENCLAW_INTERNAL_CONTEXT_BLOCK_RE, "");
}

function stripInterSessionHeader(text) {
  if (!text || typeof text !== "string") return text;
  if (!text.includes("[Inter-session message]")) return text;
  return text.replace(INTER_SESSION_HEADER_RE, "");
}
...
export function stripOpenClawInjectedPrefix(text) {
  if (!text || typeof text !== "string") return "";
  const withoutInternalContext = stripOpenClawInternalContext(text);
  const withoutInterSession = stripInterSessionHeader(withoutInternalContext).trimStart();
  const cleanedText = stripFeishuInjectedPrompt(withoutInterSession);
